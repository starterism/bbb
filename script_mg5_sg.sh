...
do
	...
	cp $txtpath/mg5_bbb_sg_$MSIX21.txt $mg5path
	cp $txtpath/mg5_bbb_sg_$MSIX22.txt $mg5path
	cd $mg5path
	./mg5_aMC mg5_bbb_sg_$MSIX21.txt
	rm -f $mg5path/mg5_bbb_sg_$MSIX21.txt

	\cp -rf $toolpath/run_delphes3 $outputpath/$MSIX2/bin/internal
	cd $mg5path
	./mg5_aMC mg5_bbb_sg_$MSIX22.txt
	rm -f $mg5path/mg5_bbb_sg_$MSIX22.txt
